...
class FluxQuery:
    """
        ...
        :param start: start time of time range, default set as '-1h', 
            here only support the relative time and absolute time formated as ISO 8601
        :param stop: stop time of time range, default set as 'now()',
             rest parts like stop time
        :param filters: dictionary of filter conditions, for emxample: 
            {"_measurement":"temperature and current of devices","device_id":4,"_field":
            ["temperature","current"]}
        :param aggregateWindow: dict that must obey the format as 
            "{"every":"3s", "fn":"mean","createEmpty":"true"}"
        :param flux_script: has the prioriry above other parameters, 
        if it is None, then function execute the flux script to query.
        """
    ...
    def set_range(self,start,stop='now()'):
        # 这里是否会有一个时间转换的问题？
        # 主要是不确定influxdb内部是将相对时间的now()以本地时间还是utc时间为基础
        'set time range'
    ...
    def set_filter(self, filters:Filter=None, **kwargs):
        'set filter conditions'

        return self.set_attribute("filters", filters, **kwargs)


    def set_aggregate_window(self,aggregate_window: AggregateWindow=None, **kwargs): # !
        'set aggregate window'

        return self.set_attribute('aggregate_window',aggregate_window,**kwargs)


    def set_pivot(self,pivot: Pivot=None, **kwargs): # !
        'set pivot'

        return self.set_attribute('pivot', pivot, **kwargs)


    def set_limit(self,limit: Limit=None, **kwargs): # !
        'set limit'
